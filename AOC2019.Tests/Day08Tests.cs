...
namespace AOC2019.Tests
{
     ...
     * process of rebooting! It's just waiting for someone to enter a
     * BIOS password. The Elf responsible for the rover takes a picture
     * of the password (your puzzle input) and sends it to you via the
     * Digital Sending Network.
     * 
     ...
     * special Space Image Format. None of the Elves seem to remember
     * why this is the case. They send you the instructions to decode it.
     * 
     * Images are sent as a series of digits that each represent the
     * color of a single pixel. The digits fill each row of the image
     ...
     * layers that are filled in this way. So, the first digit
     * corresponds to the top-left pixel of the first layer, the second
     * digit corresponds to the pixel to the right of that on the same
     * layer, and so on until the last digit, which corresponds to the
     * bottom-right pixel of the last layer.
     * 
     * For example, given an image 3 pixels wide and 2 pixels tall, the
     * image data 123456789012 corresponds to the following image
     * layers:
     * 
     * Layer 1: 123
     *          456
    ...
    public class Day08Tests : TestBase
    {
        ...
        public void TestChecksum()
        {
            ...
            Day08.Checksum(imageData, width * height).Should().Be(1);
        }
        ...
        public void Part2()
        {
            var result = Day08.RenderImage(input.readAllText(), 150);
            result.Should().Be("100011000011110111000011010001100001000010010000100101010000111001001000010001001000010000111000001000100100001000010000100100010011110100001000001100");

            //Write image to console so we can read the letters
            //Bit of a hack but writing an OCR for this is just too much!
            result.Chunk(25).ToList().ForEach(s => Console.WriteLine(s.Replace("0", " ").Replace("1", "*")));
        }
    }
}

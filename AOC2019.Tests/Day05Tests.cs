...
namespace AOC2019.Tests
{
     ...
     * Mercury. The Elves suggest that you get the air conditioner
     * working by upgrading your ship computer to support the Thermal
     ...
     * diagnostic program will run on your existing Intcode computer
     ...
     *
     * The program 3,0,4,0,99 outputs whatever it gets as input, then
     * halts.
     * 
     * Second, you'll need to add support for parameter modes:
     * 
     * Each parameter of an instruction is handled based on its
     * parameter mode. Right now, your ship computer already understands
     * parameter mode 0, position mode, which causes the parameter to be
     * interpreted as a position - if the parameter is 50, its value is
     * the value stored at address 50 in memory. Until now, all
     ...
     * opcode. The opcode is a two-digit number based only on the ones
     * and tens digit of the value, that is, the opcode is the rightmost
     * two digits of the first value in an instruction. Parameter modes
     * are single digits, one per parameter, read right-to-left from the
     ...
     * the rightmost two digits of the first value, 02, indicate opcode
     * 2, multiplication.
     *
     ...
     * to the third parameter, 4 in position mode, which also works like
     ...
     * 
     * The TEST diagnostic program will start by requesting from the
     * user the ID of the system to test by running an input instruction
     ...
     * instruction indicating how far the result of the test was from
     * the expected value, where 0 means the test was successful.
     * Non-zero outputs mean that a function is not working correctly;
     * check the instructions that were run before the output
     * instruction to see which one failed.
     * 
     * Finally, the program will output a diagnostic code and
     * immediately halt. This final output isn't an error; an output
     * followed immediately by a halt means the program finished. If all
     * outputs were zero except the diagnostic code, the diagnostic
     * program ran successfully.
     * 
     * After providing 1 to the only input instruction and passing all
     ...
     * radiators. Fortunately, the diagnostic program (your puzzle
     * input) is already equipped for this. Unfortunately, your Intcode
     * computer is not.
     * 
     ...
     *     Otherwise, it stores 0.
     *
     * Like all instructions, these instructions need to support
     * parameter modes as described above.
     * 
     * Normally, after an instruction is finished, the instruction
     * pointer increases by the number of values in that instruction.
     ...
     *     Using position mode, consider whether the input = 8;
     *     output 1 (if it is) or 0 (if it is not).
     * 3,9,7,9,10,9,4,9,99,-1,8
     *     Using position mode, consider whether the input < 8
     *     output 1 (if it is) or 0 (if it is not).
     * 3,3,1108,-1,8,3,4,3,99
     *     Using immediate mode, consider whether the input = 8
     *     output 1 (if it is) or 0 (if it is not).
     ...
     * single number. The program will then output 999 if the input
     ...
     * diagnostic test suite only outputs one number, the diagnostic
     * code.
     * 
    ...
    public class Day05Tests : TestBase
    {
        ...
        public void Part1()
        {
            ...
            results.Last().Should().Be(16225258);
        }

...
namespace AOC2019.Tests
{
     ...
     * power to your ship's thrusters to reach Santa in time. To do
     * this, you'll need to configure a series of amplifiers already
     * installed on the ship.
     * 
     * There are five amplifiers connected in series; each one receives
     * an input signal and produces an output signal. They are connected
     * such that the first amplifier's output leads to the second
     * amplifier's input, the second amplifier's output leads to the
     * third amplifier's input, and so on. The first amplifier's input
     * value is 0, and the last amplifier's output leads to your ship's
     * thrusters.
     * 
     *     O-------O  O-------O  O-------O  O-------O  O-------O
     ...
     * will first use an input instruction to ask the amplifier for its
     * current phase setting (an integer from 0 to 4). Each phase
     * setting is used exactly once, but the Elves can't remember which
     ...
     * supply it back to the amplifier with an output instruction. (If
     * the amplifier has not yet received an input signal, it waits
     * until one arrives.)
     * 
     ...
     * 3, amplifier B to setting 1, C to 2, D to 4, and E to 0. Then,
     * you could determine the output signal that gets sent from
     ...
     * A. It will then produce a new output signal destined for
     * amplifier C.
     ...
     * (phase sequence 4,3,2,1,0):
     * 3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0
     * 
     * Max thruster signal 54321:
     * (phase sequence 0,1,2,3,4):
     * 3,23,3,24,1002,24,10,24,1002,23,-1,23,
     * 101,5,23,23,1,24,23,23,4,23,99,0,0
     *
     * Max thruster signal 65210:
     * (phase sequence 1,0,4,3,2):
     * 3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,
     * 1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0
     *
     * Try every combination of phase settings on the amplifiers. What
     * is the highest signal that can be sent to the thrusters?
     * 
     * --- Part Two ---
     *
     * It's no good - in this configuration, the amplifiers can't
     * generate a large enough output signal to produce the thrust
     * you'll need. The Elves quickly talk you through rewiring the
     ...
     * amplifier A's output is connected to amplifier B's input, and so
     * on. However, the output from amplifier E is now connected into
     * amplifier A's input. This creates the feedback loop: the signal
     * will be sent through the amplifiers many times.
     * 
     * In feedback loop mode, the amplifiers need totally different
     * phase settings: integers from 5 to 9, again each used exactly
     * once. These settings will cause the Amplifier Controller Software
     * to repeatedly take input and produce output many times before
     * halting. Provide each amplifier its phase setting at its first
     * input instruction; all further input/output instructions are for
     * signals.
     * 
     * Don't restart the Amplifier Controller Software on any amplifier
     * during this process. Each one should continue receiving and
     * sending signals until it halts.
     * 
     * All signals sent or received in this process will be between
     * pairs of amplifiers except the very first signal and the very
     * last signal. To start the process, a 0 signal is sent to
     ...
     * (phase sequence 9,8,7,6,5):
     * 3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,
     * 27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5
     * 
     * Max thruster signal 18216:
     * (phase sequence 9,7,8,5,6):

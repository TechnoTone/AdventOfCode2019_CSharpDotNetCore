...
namespace AOC2019.Tests
{
     ...
     * can. Their report indicates that this was a Category 6 disaster
     * - not because it was that bad, but because it destroyed the
     * stockpile of Category 6 network cables as well as most of the
     * ship's network infrastructure.
     *
     * You'll need to rebuild the network from scratch.
     *
     * The computers on the network are standard Intcode computers that
     * communicate by sending packets to each other. There are 50 of
     * them in total, each running a copy of the same Network Interface
     * Controller (NIC) software (your puzzle input). The computers have
     * network addresses 0 through 49; when each computer boots up, it
     * will request its network address via a single input instruction.
     ...
     * Packets sent to a computer are queued by the recipient and read
     * in the order they are received.
     *
     * To send a packet to another computer, the NIC will use three
     * output instructions that provide the destination address of the
     * packet followed by its X and Y values. For example, three output
     ...
     * input instruction. If the incoming packet queue is empty, provide
     * -1. Otherwise, provide the X value of the next packet; the
     * computer will then use a second input instruction to receive the
     * Y value for the same packet. Once both values of the packet are
     ...
     * is, the data in each packet it receives overwrites the NAT's
     ...
     * trying to receive packets without sending packets, the network
     * is considered idle.
     *
     * Once the network is idle, the NAT sends only the last packet it
     * received to address 0; this will cause the computers on the
     * network to resume activity. In this way, the NAT can throttle
     * power consumption of the network when the ship needs power in
     * other areas.
     *
    ...
    public class Day23Tests : TestBase
    {
        ...
        public void Part1()
        {
            var day23 = new Day23(input.readAllText());
            var result = day23.GetFirstPacketSentTo(255);

            result.Y.Should().Be(17286);
        }
        ...
        {
            var day23 = new Day23(input.readAllText());
            var result = day23.FirstDuplicateNatYValue();

            result.Should().Be(11249);
        }
    }
}

...
namespace AOC2019.Tests
{
     ...
     * calculate his position in space, safely align his warp
     * drive, and return to Earth in time to save Christmas, he
     ...
     * Counter-Upper. They haven't determined the amount of fuel
     * required yet.
     *
     ...
     * Apparently, you forgot to include additional fuel for the
     * fuel you just added.
     *
     * Fuel itself requires fuel just like a module - take its mass,
     * divide by three, round down, and subtract 2. However, that
     * fuel also requires fuel, and that fuel requires fuel, and so
     * on. Any mass that would require negative fuel should instead
     * be treated as if it requires zero fuel; the remaining mass,
     * if any, is instead handled by wishing really hard, which has
     * no mass and is outside the scope of this calculation.
     *
     * So, for each module mass, calculate its fuel and add it to
     * the total. Then, treat the fuel amount you just calculated
     * as the input mass and repeat the process, continuing until a
     ...
     * 5 fuel, which requires no further fuel.
     *
     ...
     * mass of the added fuel? (Calculate the fuel requirements for
     * each module separately, then add them all up at the end.)
     */

    public class Day01Tests : TestBase
    {
        private readonly List<int> moduleMasses;
        ...
        public void Part1Examples(int mass, int fuel) =>
            Day01.fuelFor(mass).Should().Be(fuel);

        [Test]
        [TestCase(14, 2)]
        [TestCase(1969, 966)]
        [TestCase(100756, 50346)]
        public void Part2Examples(int mass, int fuel) =>
            Day01.correctFuelFor(mass).Should().Be(fuel);

        [Test]
        public void Part1() =>
            moduleMasses.ConvertAll(Day01.fuelFor).Sum()
                .Should().Be(3336439);

        [Test]
        public void Part2() =>
            moduleMasses.ConvertAll(Day01.correctFuelFor).Sum()
                .Should().Be(5001791);
    }
}

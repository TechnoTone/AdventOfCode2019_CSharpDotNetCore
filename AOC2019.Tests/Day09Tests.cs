...
namespace AOC2019.Tests
{
     ...
     * sensors, for tenuous safety reasons, it refuses to do so until
     * the computer it runs on passes some checks to demonstrate it is a
     ...
     * is invalid to try to access memory at a negative address,
     * though.)
     *
     ...
     *
     * 1102,34915192,34915192,7,4,7,99,0 should output 16-digit number.
     * 104,1125899906842624,99 should output the large number.
     *
     ...
     * will output a single value: the coordinates of the distress
     * signal.
     *
    ...
    public class Day09Tests : TestBase
    {
        ...
        public void Part1()
        {
            var output = Day09.runTest(input.readAllText());

            // output.ForEach(x => HelperFunctions.Log(x.ToString()));

            output.Count.Should().Be(1);
            output[0].Should().Be(4006117640);
        }

        [Test]
        public void Part2()
        {
            var output = Day09.runBoost(input.readAllText());

            output.Count.Should().Be(1);
            output[0].Should().Be(88231);
        }
    }
}

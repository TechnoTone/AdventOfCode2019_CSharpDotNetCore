...
namespace AOC2019.Tests
{
     ...
     *
     * "Attention, unmarked spacecraft! You are in violation of Space
     * Law! All spacecraft must have a clearly visible registration
     * identifier! You have 24 hours to comply or be sent to Space
     * Jail!"
     *
     ...
     * reply signal to reach you, they send instructions for how to
     * power up the emergency hull painting robot and even provide a
     * small Intcode program (your puzzle input) that will cause it to
     * paint your ship appropriately.
     *
     * There's just one problem: you don't have an emergency hull
     * painting robot.
     *
     * You'll need to build a new emergency hull painting robot. The
     * robot needs to be able to move around on the grid of square
     * panels on the side of your ship, detect the color of its current
     * panel, and paint its current panel black or white. (All of the
     * panels are currently black.)
     *
     ...
     *
     * The robot will continue running for a while like this and halt
     * when it is finished drawing. Do not restart the Intcode computer
     * inside the robot during this process.
     *
     ...
     * direction it is facing (< ^ > v), the initial state and region
     ...
     * instead) is also black, and so any input instructions at this
     * point should be provided 0. Suppose the robot eventually outputs
     * 1 (paint white) and then 0 (turn left). After taking these
     * actions and moving forward one panel, the region now looks like
     * this:
     *
     * .....
     * .....
     ...
     * not a registration identifier. The Space Police are getting
     * impatient.
     *
     ...
     * rest of the panels are still black, but it looks like the robot
     ...
     * single white panel instead, what registration identifier does it
     * paint on your hull?
     *
    ...
    public class Day11Tests : TestBase
    {
        ...
        [Test]
        public void Part1() =>
            new Day11(input.commaSeparatedIntegers()).Run(0).Should().HaveCount(2054);
        ...
        public void Part2()
        {
            ...
            surface.Should().BeEquivalentTo(expected);

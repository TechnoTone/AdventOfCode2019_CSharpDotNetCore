...
namespace AOC2019.Tests
{
     ...
     * computer beeps angrily about a "1202 program alarm". On the
     * radio, an Elf is already explaining how to handle the situation:
     * "Don't worry, that's perfectly norma--" The ship computer bursts
     * into flames.
     *
     * You notify the Elves that the computer's magic smoke seems to
     * have escaped. "That computer ran Intcode programs like the
     * gravity assist program it was working on; surely there are
     ...
     * example, 99 means that the program is finished and should
     * immediately halt. Encountering an unknown opcode means
     ...
     * it should read the values at positions 10 and 20, add those
     * values, and then overwrite the value at position 30 with
     * their sum.
     *
     ...
     * 30,40,50
     * 
     * The first four integers, 1,9,10,3,
     * are at positions 0, 1, 2, and 3.
     ...
     * 40. Add these numbers together to get 70. Then, store this value
     * at the output position; here, the output position (3) is at
     ...
     * works just like the previous, but it multiplies instead of adding.
     * The inputs are at positions 3 and 11; these positions contain 70
     * and 50 respectively. Multiplying these produces 3500; this is
     ...
     * Intcode computers support many more features than your new one,
     ...
     *
     * Intcode programs are given as a list of integers; these values
     * are used as the initial state for the computer's memory. When you
     * run an Intcode program, make sure to start by initializing memory
     * to the program's values. A position in memory is called an
     * address (for example, the first value in memory is at
     * "address 0").
     *
     * Opcodes (like 1, 2, or 99) mark the beginning of an instruction.
     * The values used immediately after an opcode, if any, are called
     * the instruction's parameters. For example, in the instruction
     * 1,2,3,4, 1 is the opcode; 2, 3, and 4 are the parameters. The
     ...
     * instruction; until you add more instructions to the computer,
     * this is always 4 (1 opcode + 3 parameters) for the add and
     * multiply instructions. (The halt instruction would increase the
     ...
     * the value placed in address 2 is called the verb. Each of the two
     * input values will be between 0 and 99, inclusive.
     *
     * Once the program has halted, its output is available at address
     * 0, also just like before. Each time you try a pair of inputs,
     * make sure you first reset the computer's memory to the values in
     * the program (your puzzle input) - in other words, don't reuse
    ...
    public class Day02Tests : TestBase
    {
        ...
        public void Part2()
        {
            var day02 = new Day02(input.commaSeparatedIntegers());
            var answer = day02.findSolution(19690720);
